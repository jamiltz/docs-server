...
xref:clustersetup:setup-failover-graceful.adoc#topic_ysk_ycm_zs[Graceful failover]::
...
xref:clustersetup:hard-failover.adoc#topic_a4s_d24_vs[Hard failover]::
...
xref:clustersetup:automatic-failover.adoc#topic_fcf_chm_zs[Automatic failover]:: Automatic failover is the built-in ability to have the Cluster Manager detect and determine when a node is unavailable and then initiate a hard failover.

Keep in mind that each of the Couchbase Services reacts differently to failover.
For example, if you have the services separated on different nodes using xref:clustersetup:services-mds.adoc#concept_evc_1vz_nr[Multi-Dimensional Scaling (MDS]), you can failover and recover nodes separately.
...
Automatic failover::
With the xref:clustersetup:automatic-failover.adoc#topic_fcf_chm_zs[automatic failover], the Cluster Manager handles the detection, determination of, and initiation of the processes to failover a node without user intervention and without identification of the issue that caused the node failure.
...
+
If you do not use automatic failover and a node becomes unhealthy, it will be up to the administrators to intervene and initiate a xref:clustersetup:hard-failover.adoc#topic_a4s_d24_vs[hard failover] of the node.
