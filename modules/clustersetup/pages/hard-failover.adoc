...
For that purpose see the explanations provided in xref:clustersetup:setup-failover-graceful.adoc#topic_ysk_ycm_zs[Using Graceful Failover] or xref:clustersetup:remove-nodes.adoc#topic_byh_lbg_q4[Removing a Node].
...
To read more about this, see xref:clustersetup:automatic-failover.adoc#topic_fcf_chm_zs[Using Automatic Failover].
...
* The other option is to xref:clustersetup:adding-nodes.adoc#topic_ncd_gdg_q4[add a node] and xref:clustersetup:rebalance.adoc#topic_xsx_1mn_vs[rebalance the cluster].
