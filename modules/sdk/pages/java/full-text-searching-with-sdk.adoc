...
A general introduction to Full Text Search, with pointers to detailed descriptions of its principal features, is provided in xref:..:fts/full-text-intro.adoc[Full Text Search: Fundamentals].
...
For information on creating users and managing roles, see xref:..:security/security-authorization.adoc[Authorization].
For information on installing sample buckets, see xref:..:settings/settings.adoc[Settings].
...
See xref:..:fts/fts-creating-indexes.adoc[Creating Indexes] for details on how to create these indexes: they can be created interactively, by means of the Couchbase Web Console; however, there may be greater efficiency in using the Couchbase REST API, as described in the section xref:..:fts/fts-creating-indexes.adoc#index-creation-with-the-rest-api[Index Creation with the REST API].
The JSON objects that constitute index-definitions (for inclusion as bodies to the index-creation REST calls), are provided in xref:..:fts/fts-demonstration-indexes.adoc[Demonstration Indexes].
