...
xref:install:upgrade-strategy-for-features.adoc#topic_umb_kzr_tdb[Upgrading to Couchbase Server 5.5] lists the new features in this release and their upgrade behavior.
...
Follow xref:install:upgrade-strategies.adoc#intercluster[Upgrading Using Intercluster Replication], having in mind that the new cluster would be the older version to which you are rolling back.
...
For more information on supported upgrade paths of Couchbase Server, see the xref:install:upgrade-matrix.adoc#topic_dwm_qfv_xs[upgrade matrix.]
...
For instructions to uninstall Couchbase Server, see xref:install:install-uninstalling.adoc#concept_ixr_wsp_ts[Uninstalling Couchbase Server].
