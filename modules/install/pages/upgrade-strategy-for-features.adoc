...
Follow the instructions available in the section xref:install:upgrade-offline.adoc#topic_q11_1my_p4[Performing the Offline Upgrade].
When all the nodes are offline, restart each node with IPv6 enabled.
See xref:install:ipv6-setup.adoc#ipv6-setup[IP Version 6: Set-Up Procedures] for instructions to enable IPv6.
