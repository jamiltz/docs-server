...
+
See xref:install:rhel-suse-install-intro.adoc#rh-nonroot-nonsudo-[Installing as non-root, non-sudo]for RHEL/CentOS and xref:install:ubuntu-debian-install.adoc#deb-nonroot-nonsudo[Installing as non-root, non-sudo] for Ubuntu/Debian.
