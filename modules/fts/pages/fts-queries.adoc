...
All the above options, and others, are explained in detail on the page xref:fts:fts-query-types.adoc[Query Types].

For information on how to execute queries, see xref:fts:fts-performing-searches.adoc[Performing Searches].

[#sorting-results]
== Sorting Results

The results returned from a Full Text Search can be _sorted_: by *id*, *score*, *field*, and more.
Details are provided in xref:fts:fts-sorting.adoc[Sorting Query Results].
...
For full information, see xref:fts:fts-response-object-schema.adoc[Handling Response Objects].
