...
Detailed instructions for setting up indexes, and specifying type mappings, are provided in xref:fts:fts-creating-indexes.adoc[Creating Indexes].
...
See xref:fts:fts-demonstration-indexes.adoc#topic_fts_demonstration-indexes[Demonstration Indexes] for the body of the index to be used, and see xref:fts:fts-creating-indexes.adoc#index-creation-with-the-rest-api[Index-Creation with the REST API] for information on using the REST syntax.
...
Note that more detailed information on performing queries with the Couchbase REST API can be found in xref:fts:fts-searching-with-the-rest-api.adoc[Searching with the REST API]; which shows how to use the full `curl` command, and how to incorporate query-bodies into it.
