...
Additionally, it explains how index-creation can be achieved with the Couchbase REST API: see xref:fts:fts-creating-indexes.adoc#index-creation-with-the-rest-api[Index-Creation with the REST API], below.

[#creating-a-full-text-index]
== Creating a Full Text Index

Index-creation can be performed by means of the Couchbase Web Console.
The basic procedure is outlined in xref:fts:fts-searching-from-the-ui.adoc[Searching from the UI].
...
(See xref:fts:fts-searching-from-the-ui.adoc[Searching from the UI] for a full illustration.) To manage an index, left-click on its row.
...
+
Note that both the [.uicontrol]*Edit Index* and [.uicontrol]*Clone Index* screens are in most respects the same as the [.uicontrol]*Add Index* screen, which was itself described in xref:fts:fts-searching-from-the-ui.adoc[Searching from the UI].
...
For information on how values are data-typed when dynamic mapping is specified, see the section below, xref:fts:fts-creating-indexes.adoc#document-fields-and-data-types[Document Fields and Data Types].
...
(For an example, see xref:fts:fts-creating-indexes.adoc#specifying-fields[Specifying Fields], below.)

Optionally, an _analyzer_ can be specified for the type mapping: for all queries that do indeed support use of an analyzer, the specified analyzer will be applied, rather than the default analyzer (which is itself specified in the *Advanced* pane, as described below, in xref:fts:fts-creating-indexes.adoc#specifying-advanced-settings[Specifying Advanced Settings]).
...
Note that custom analyzers can be created and stored for the index that is being defined, by means of the [.uicontrol]*Analyzers* panel, described below in xref:fts:fts-creating-indexes.adoc#creating-analyzers[Creating Analyzers].
...
Additional information on analyzers can also be found on the page xref:fts:fts-using-analyzers.adoc[Understanding Analyzers].
...
For more information on analyzers, see xref:fts:fts-using-analyzers.adoc[Understanding Analyzers].
...
{blank}
+
For an explanation of character filters, see the section in xref:fts:fts-using-analyzers.adoc#character-filters[Understanding Analyzers].
...
+
For more information on tokenizers, see the section in xref:fts:fts-using-analyzers.adoc#character-filters[Understanding Analyzers].
...
+
For more information on token filters, see the section in xref:fts:fts-using-analyzers.adoc#character-filters[Understanding Analyzers].
...
For descriptions, see the section xref:fts:fts-using-analyzers.adoc#token-filters[Token Filters], on the page xref:fts:fts-using-analyzers.adoc[Understanding Analyzers].
...
Note that the type of an index is saved in its JSON definition, which can be previewed in the xref:fts:fts-creating-indexes.adoc#index-definition-preview[Index Definition Preview] panel, at the right-hand side.
...
The simplest way to create the appropriate JSON index-definition for the body is to create an index by means of the Couchbase Web Console, make a copy of the JSON index-definition thereby produced (by accessing the xref:fts:fts-creating-indexes.adoc#using-the-index-definition-preview[Using the Index Definition Preview], explained above), modify the index-definition as appropriate, and finally, add the index-definition to the other, preceding elements required for the call.
