...
To demonstrate this, follow the procedures for accessing the Full Text Search screen, within the Couchbase Web Console, and for performing a simple search; as described in xref:fts:fts-searching-from-the-ui.adoc[Searching from the UI].
...
A detailed description of Query String-format is provided in xref:fts:fts-query-types.adoc[Query  Types].
...
Further examples of using the REST API to conduct Full Text Searches can be found in xref:fts:fts-query-types.adoc[Query Types].
