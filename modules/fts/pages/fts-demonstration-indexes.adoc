...
Information on this process is provided in xref:fts:fts-creating-indexes.adoc[Creating Indexes].
...
Information on doing so can be found in the section xref:fts:fts-creating-indexes.adoc#index-creation-with-the-rest-api[Index-Creation with the REST API], on the _Creating Indexes_ page.
The three definitions are provided below.

This page also provides a definition for the `geoIndex` definition, used in the section xref:fts:fts-geospatial-queries.adoc[Geospatial Queries].
...
The following index is used to support the _geospatial_ queries described in xref:fts:fts-geospatial-queries.adoc[Geospatial Queries].
