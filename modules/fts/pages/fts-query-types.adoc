...
For more information on using the REST API to perform queries, see xref:fts:fts-searching-with-the-rest-api.adoc[Searching with the REST API].
...
For information on analyzers, see xref:fts:fts-using-analyzers.adoc[Understanding Analyzers].
+
...
(See xref:fts:fts-searching-from-the-ui.adoc[Searching from the UI].)
...
More detailed information is provided in xref:fts:fts-geospatial-queries.adoc[Geospatial Queries].
