...
To review the wide range of available options for creating indexes appropriate for production environments, see xref:fts:fts-creating-indexes.adoc[Creating Indexes].
...
See xref:fts:fts-searching-with-the-rest-api.adoc[Searching with the REST API] for an example.)
...
Additional information on Query String searches is also provided in xref:fts:fts-query-types.adoc[Query Types].
