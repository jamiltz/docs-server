...
This timing differs from xref:cli:cbstats/cbstats-timing.adoc[cbstats timings], which only measures the time spent on a request by a front-end worker thread and does not include any time spent fetching items from disk.
