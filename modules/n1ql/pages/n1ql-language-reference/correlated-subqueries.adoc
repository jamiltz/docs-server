...
In the examples xref:n1ql:subqueries.adoc#Q1[Q1], xref:n1ql:subqueries.adoc#Q2[Q2], and xref:n1ql:subqueries.adoc#Q3[Q3], subqueries are independent of the parent queries.
...
See xref:n1ql:subqueries.adoc#nested-path-expr[Nested Paths in Subqueries] for more details.
+
{blank}
*Better Performance *
...
This significantly improves the performance of such subqueries, as shown in example xref:n1ql:subqueries.adoc#Q6[Q6] earlier, to contrast the example xref:n1ql:subqueries.adoc#Q6A[Q6A] cannot take advantage of this optimization.

Correlation by Reference (or non FROM clause-Expression)::
In this case the subqueries refer to xref:n1ql:subqueries.adoc#section_onz_3tj_mz[variables in the scope] of outer level queries, in clauses other than the FROM clause of the subquery.
...
For more information, see <<use-keys,USE KEYS in the Subquery>> and xref:n1ql:subqueries.adoc#from-clause[FROM clause in Subqueries] for more details.
...
For example:
[#ul_oxc_jqx_mz]
* FROM clause of the subquery in xref:n1ql:subqueries.adoc#Q7[Q7] is an independent keyspace identifier ``travel-sample`` and hence the correlation with parent query is established explicitly using the USE KEYS clause through the referential attribute `t1.airlineid`.
[#ul_pxc_jqx_mz]
* Similarly, the subquery in xref:n1ql:subqueries.adoc#Q6A[Q6A] has an independent keyspace identifier ``travel-sample`` in FROM clause, but the correlation is self-referencing to the same document.
Therefore, `USE KEYS meta(t).id` is used.

This is exactly same as the PrimaryKey-ForeignKey relationship required to xref:n1ql:from.adoc#join-clause[join] two documents that are referenced in the outer/inner queries.
...
In the example xref:n1ql:subqueries.adoc#Q6[Q6], the FROM clause is an expression referring to the variable/alias `t` (in fact the nested path `t.reviews`) that already establishes correlation and henc the subquery does not need explicit USE KEYS clause.
