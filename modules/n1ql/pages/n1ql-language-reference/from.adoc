...
----
+
Note that functions such as xref:n1ql:curl.adoc#topic_8_8[CURL()] can independently produce input data objects for the query.
Similarly, other N1QL functions can also be used in the expressions.

*2. Variable N1QL Expression*:: 
This includes expressions that refer to any xref:n1ql:subqueries.adoc#section_onz_3tj_mz[variables in scope] for the query, for example:
+
----
SELECT count(*) 
...
This makes the subquery correlated with outer queries, as explained in the xref:n1ql:subqueries.adoc#topic_9[Subqueries] section.
...
----
+
For more details and examples, see xref:n1ql:subqueries.adoc#topic_9[Subqueries] and ( select-expr ).
...
For more information on aliases, see xref:n1ql:identifiers.adoc[Identifiers].
====
...
NOTE: Couchbase Server version 4.6.2 adds support for http://docs-build.sc.couchbase.com/server/2059/n1ql/n1ql-language-reference/from.html#concept_rnt_zfk_np__section_fkc_ftt_nz[+++<u>generic expression</u>+++] in the from-term.
...
[#image_p21_3vz_1db]
image::FROM_ansi-join_RR-clause_5.5.png[]
...
Visual Explain Plan:image:FROM_AnsiJoin-Ex4-BeerVisual1.png[]
...
[#image_fxp_gk1_bdb]
image::FROM_AnsiJoin-Ex4-BeerVisual2.png[]
...
[#image_asp_x3h_bdb]
image::FROM_lookup-join-clause.png[]
...
[#image_cdn_bc3_bdb]
image::FROM_index-join-clause.png[]
...
[#image_sg1_dd3_bdb]
image::FROM_unnest-clause_4.0_RR.png[]
...
NOTE: ANSI NEST (and https://docs.google.com/document/d/10IfKfrsn6u5DNFjO5FNMKjlPV6fJ79u7Ezu3b52koi4/edit#heading=h.byr1fhhe7uur[+++<u>ANSI JOIN</u>+++]) clauses are much faster and have much more flexible functionality than their earlier INDEX and LOOKUP equivalents, so users are strongly recommended to use ANSI NEST (and ANSI JOIN) exclusively, where possible.
...
[#image_wwl_cf3_bdb]
image::FROM_ansi-nest_RR-clause_5.5.png[]
...
[#image_nxv_yg3_bdb]
image::FROM_lookup-nest_4.0_RR.png[]
...
[#image_emh_343_bdb]
image::FROM_index-nest_5.1_rr.png[]
...
For details, see http://docs-build.sc.couchbase.com/server/2059/n1ql/n1ql-language-reference/from.html#concept_rnt_zfk_np__keyspaces[+++<u>Keyspaces</u>+++].

`AS` alias::
[Optional] To assign another name.
For details, see http://docs-build.sc.couchbase.com/server/2059/n1ql/n1ql-language-reference/from.html#concept_rnt_zfk_np__as[+++<u>AS Keyword</u>+++].
