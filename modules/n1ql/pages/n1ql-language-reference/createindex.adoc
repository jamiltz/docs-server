...
----
IMPORTANT: Indexes cannot be built concurrently on a given bucket unless the `defer_build` option of the CREATE INDEX statement is used in combination with BUILD INDEX statement.
...
See xref:n1ql:indexing-arrays.adoc[Array Indexing] for details.
...
For details about array expressions, see xref:n1ql:indexing-arrays.adoc[Array Indexing].
...
For details, see xref:n1ql:indexing-meta-info.adoc#untitled4[Indexing Meta Info].
