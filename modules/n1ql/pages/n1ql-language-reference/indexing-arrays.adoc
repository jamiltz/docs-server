...
[.var]`expr`:::
A xref:n1ql:index.adoc#n1ql-lang-ref[N1QL expression] over any fields in the document.
...
See the xref:n1ql:createindex.adoc#concept_npk_mth_rq[CREATE INDEX] statement for more details on the syntax.
...
For example, the following query Q7C uses the non-array index` def_type `ignoring the xref:n1ql:hints.adoc#concept_npj_srf_2s[USE INDEX hint] to use the array indexes (note that query C7 defines a DISTINCT array index while C7C defines an ALL array index, and both are ignored).
