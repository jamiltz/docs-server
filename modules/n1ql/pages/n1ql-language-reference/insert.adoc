...
TIP: Use the xref:n1ql:upsert.adoc#topic_11_9[UPSERT] statement if you want to overwrite a document with the same key, in case it already exists.
...
For more information on aliases, see xref:n1ql:identifiers.adoc#identifier-alias[Aliases].
...
*Example 3*: If you don’t require the document key to be in a specific format, you can use the function xref:n1ql:metafun.adoc#topic_8_8[UUID()] to generate a unique key as shown here:
...
For details, see xref:n1ql:select-syntax.adoc#topic_vr1_zzk_dx[SELECT Syntax].
...
For more information, see xref:n1ql:identifiers.adoc#topic_6[Identifiers].
...
For detailed explanation about the EXPLAIN plan, see the xref:n1ql:explain.adoc#topic_11_4[EXPLAIN] statement.
