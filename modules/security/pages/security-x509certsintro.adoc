...
Full administrators can manage certificates, using the Couchbase CLI tools (as described in xref:cli:cbcli/couchbase-cli-ssl-manage.adoc[cli:cbcli/couchbase-cli-ssl-manage.adoc]) and REST API (as described in xref:rest-api:rest-security.adoc#reference_ytt_hdx_hv[Security API]).
...
(See xref:security:security-authorization.adoc[Authorization] for more information on RBAC.)
+
{blank}
+
{blank}
...
For an overview, see xref:security:security-certs-auth.adoc[Certificate-Based Authentication]
