= X.509 for TLS
:gen_keystore: {examplesdir}/gen_keystore.sh
:openssl-san: {examplesdir}/openssl-san.cnf
:v3_ca: {examplesdir}/v3_ca.ext
...
This section explains how to configure X.509 certificates for TLS in Couchbase Server on Ubuntu 16.

A root certificate is created with a single intermediate certificate and a single node certificate; and a chain certificate is created from the intermediate and node certificates.
...
[source,bash]
----
couchbase-cli ssl-manage -c ${ip}:8091:8091 \
              -u Administrator -p password \
              --upload-cluster-ca=./${ROOT_DIR}/${ROOT_CA}.pem

couchbase-cli ssl-manage -c ${ip}:8091 \
              -u Administrator -p password \
              --set-node-certificate
----
--
====
. If the previous operation is successful, the certificate's fields and fingerprint are displayed on the Root Certificate tab of the Couchbase Console.
+
image::signed-cert-ui.png[]
...
Once the root certificate for a Couchbase Server-node has been deployed, a client can authenticate by means of an appropriately prepared keystore, and so gain access to the Data, Query, and Search Services.

For an overview, see xref:security-certs-auth.adoc[Certificate-Based Authentication]

Proceed as follows.
Note that these instructions assume use of the Ubuntu 16 environment configured in the preceding section, <<sslconfig, Configure X.509>>.
