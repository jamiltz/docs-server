...
Resources are described in xref:security:security-resources-under-access-control.adoc[Resources under Access Control].
+
{blank}
...
The associations of privileges to resources and roles are explained in xref:security:security-roles.adoc[Roles].
+
{blank}

* _Role_: An entity associated with a fixed set of privileges.
The association of privileges to roles is described in xref:security:security-roles.adoc[Roles].
+
{blank}
...
For information on how to manage users, see xref:security:security-rbac-user-management.adoc[RBAC User Management].
...
See xref:security:security-authentication.adoc[Authentication], for more information.
